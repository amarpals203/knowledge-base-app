...
//#region server/routes/lakebase/todo-routes.ts
...
async function setupSampleLakebaseRoutes(appkit) {
	try {
		const { rows } = await appkit.lakebase.query(TABLE_EXISTS_SQL);
		if (rows.length > 0) console.log("[lakebase] Table app.todos already exists, skipping setup");
		else {
		...
		console.warn("[lakebase] Database setup failed:", err.message);
	...
	appkit.server.extend((app) => {
		app.get("/api/lakebase/todos", async (_req, res) => {
			try {
				const result = await appkit.lakebase.query("SELECT id, title, completed, created_at FROM app.todos ORDER BY created_at DESC");
		...
		app.post("/api/lakebase/todos", async (req, res) => {
			try {
				...
				const result = await appkit.lakebase.query("INSERT INTO app.todos (title) VALUES ($1) RETURNING id, title, completed, created_at", [parsed.data.title.trim()]);
		...
		app.patch("/api/lakebase/todos/:id", async (req, res) => {
			try {
				...
				const result = await appkit.lakebase.query("UPDATE app.todos SET completed = NOT completed WHERE id = $1 RETURNING id, title, completed, created_at", [id]);
		...
		app.delete("/api/lakebase/todos/:id", async (req, res) => {
			try {
				...
				if ((await appkit.lakebase.query("DELETE FROM app.todos WHERE id = $1 RETURNING id", [id])).rows.length === 0) {
					res.status(404).json({ error: "Todo not found" });
					return;
				}
				res.status(204).send();
...
//#endregion
export { setupSampleLakebaseRoutes };